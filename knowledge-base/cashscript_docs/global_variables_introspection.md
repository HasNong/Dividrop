Topic: Global Variables & Transaction Introspection
Source: CashScript Website
Type: Documentation
Priority: High
Description: Complete guide to CashScript global transaction inspection variables (tx.inputs, tx.outputs, tx.version, tx.locktime) and evaluation context.
...
Because of the way time locks work, **a corresponding time lock needs to be added to the transaction**. This can be done with the CashScript SDK by calling [`setLocktime()`](typescript_sdk_transaction_builder.md#setlocktime) on the `TransactionBuilder` instance.
...
Represents the `nLocktime` field of the transaction. This is similar to the [`tx.time`](#txtime) global variable but `tx.time` can only be used in `require` statements, not for variable declaration.

> [!TIP]
> The use case for `tx.locktime` is to read the `nLocktime` value and add to the local state. Example usage for this is demonstrated in the [Sablier example](covenants_guide.md#keeping-local-state-in-nfts).
