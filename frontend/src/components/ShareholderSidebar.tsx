...
const ITEMS = [
  ...
  { id: 'portfolio', label: 'Portfolio', icon: 'M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4' },
...
export default function ShareholderSidebar({ tab, onTabChange }: Props) {
  return (
    <aside className="w-56 flex-shrink-0 flex flex-col border-r border-gray-200 bg-white sticky top-16 h-[calc(100vh-4rem)]">
      <nav className="flex-1 p-3 space-y-1">
        {ITEMS.map((item) => (
          <button key={item.id} onClick={() => onTabChange(item.id)}
            className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-all ${
              tab === item.id ? 'bg-brand-50 text-brand-700 shadow-sm' : 'text-gray-600 hover:bg-gray-50 hover:text-gray-900'
            }`}>
